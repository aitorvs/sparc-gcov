...
{
    printf("%s: %s\n", __func__, gcov_var.filename);
    bzero(gcov_var.filename, strlen(gcov_var.filename));
...
static void gcov_write_block (unsigned size)
{
    ...
    bzero(header_s.filename, MAXFILENAME);
    strcpy(header_s.filename, gcov_var.filename);
    header_s.nbytes = (size << 2);

    //    if (fwrite (gcov_var.buffer, size << 2, 1, gcov_var.file) != 1)
    //        gcov_var.error = 1;
    printf("%s: %d chars\n",__func__,  strlen(header_s.filename));
//    send_data((unsigned char *)(&header_s), sizeof(struct _to_send));
//    send_data((unsigned char *)gcov_var.buffer,size << 2);


    // send the header information of the file
    if ( (bw = if_write(dev_id, sizeof(struct _to_send), (void*)(&header_s))) != sizeof(struct _to_send))
        printf("%s: error sending the gcov data through the link\n", __func__);
    
    // Send the data related to the file
    if ( (bw = if_write(dev_id, size << 2, (void*)(gcov_var.buffer))) != (size << 2))
        printf("%s: error sending the gcov data through the link\n", __func__);
...
void gcov_if_init(void)
{
    if_init();

    if( (dev_id = if_register(&tty_operations)) < 0 )
    {
        printf("%s: error registering the output device\n", __func__);
    }
}
